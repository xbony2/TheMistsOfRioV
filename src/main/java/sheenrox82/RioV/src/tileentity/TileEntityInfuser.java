	...
	private boolean canSmelt(){
		if(this.infuserStacks[0] == null){
			...
			if(this.infuserStacks[2] == null)
				return true;
			if(!this.infuserStacks[2].isItemEqual(itemstack))
				return false;
			int result = infuserStacks[2].stackSize + itemstack.stackSize;
			return result < getInventoryStackLimit() && result <= this.infuserStacks[2].getMaxStackSize(); // Forge
																											// BugFix:
																											// Make
																											// it
																											// respect
	...
	public void smeltItem(){
		if(this.canSmelt()){
			...
			if(this.infuserStacks[2] == null){
				...
				this.infuserStacks[2].stackSize += itemstack.stackSize; // Forge
																		// BugFix:
																		// Results
																		// may
																		// have
