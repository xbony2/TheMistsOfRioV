...
public class RioVPotions extends ItemBucket{
		...
		if(this.isFull == Blocks.air){
			...
			if(!par1World.isAirBlock(par2, par3, par4) && !flag){
				...
				if(par1World.provider.isHellWorld && this.isFull == RioVBlocks.sanctuatiteMoving){
					par1World.playSoundEffect((double) ((float) par2 + 0.5F), (double) ((float) par3 + 0.5F), (double) ((float) par4 + 0.5F),
							"random.fizz", 0.5F, 2.6F + (par1World.rand.nextFloat() - par1World.rand.nextFloat()) * 0.8F);
					
					for(int l = 0; l < 8; ++l){
						par1World.spawnParticle("largesmoke", (double) par2 + Math.random(), (double) par3 + Math.random(),
	...
	public void registerIcons(IIconRegister i){
		this.itemIcon = i.registerIcon(Util.MOD_ID + ":" + RioVAPI.getInstance().getUtil().getName(this.getUnlocalizedName()));
	}
}
