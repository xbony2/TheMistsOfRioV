...
public class RioVSummoners extends Item{
	...
	@Override
	public boolean onItemUse(ItemStack par1ItemStack, EntityPlayer entityplayer, World world, int par4, int par5, int par6, int par7, float par8,
				...
				if(par1ItemStack.getItem().equals(RioVItems.terronCrystal)){
					...
					if(!world.isRemote){
						if(player.getCurrentEos() > 3){
							...
							if(player.getCurrentEos() < 3){
								if(!world.isRemote)
									entityplayer.addChatMessage(RioVAPI.getInstance().getUtil()
											.addChatMessage(EnumChatFormatting.WHITE, LangHelper.translate("msg.riov.notEnoughEos")));
							}
							
							if(player.getCurrentEos() < 0){
								if(!world.isRemote)
									entityplayer.addChatMessage(RioVAPI.getInstance().getUtil()
											.addChatMessage(EnumChatFormatting.WHITE, LangHelper.translate("msg.riov.Error2001")));
							}
						}
					}
	...
	public void addInformation(ItemStack var1, EntityPlayer var2, List var3, boolean var4){
		...
		if(var1.getItem().equals(RioVItems.woodElfKingCrystal) || var1.getItem().equals(RioVItems.altruCrystal)
				|| var1.getItem().equals(RioVItems.tiTunSummoner)){
