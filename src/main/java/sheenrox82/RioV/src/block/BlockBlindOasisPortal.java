	...
	public void onEntityCollidedWithBlock(World par1World, int par2, int par3, int par4, Entity par5Entity){
		if((par5Entity.ridingEntity == null) && (par5Entity.riddenByEntity == null) && ((par5Entity instanceof EntityPlayerMP))){
			...
			if(player.timeUntilPortal > 0){
				...
				player.timeUntilPortal = 300;
				player.mcServer.getConfigurationManager().transferPlayerToDimension(player, Config.blindOasisID,
						new TeleporterBlindOasis(player.mcServer.worldServerForDimension(Config.blindOasisID)));
			}else{
				player.timeUntilPortal = 300;
				player.mcServer.getConfigurationManager().transferPlayerToDimension(player, 0,
						new TeleporterBlindOasis(player.mcServer.worldServerForDimension(0)));
			}
		...
		protected int func_150858_a(){
			...
			for(this.field_150862_g = 0; this.field_150862_g < 21; ++this.field_150862_g){
				...
				for(j = 0; j < this.field_150868_h; ++j){
					...
					if(j == 0){
						block = this.field_150867_a.getBlock(k + Direction.offsetX[BlockBlindOasisPortal.field_150001_a[this.field_150865_b][0]], i,
								l + Direction.offsetZ[BlockBlindOasisPortal.field_150001_a[this.field_150865_b][0]]);
						
						if(block != RioVBlocks.blackQuartz){
							break label56;
						}
					}else if(j == this.field_150868_h - 1){
						block = this.field_150867_a.getBlock(k + Direction.offsetX[BlockBlindOasisPortal.field_150001_a[this.field_150865_b][1]], i,
								l + Direction.offsetZ[BlockBlindOasisPortal.field_150001_a[this.field_150865_b][1]]);
