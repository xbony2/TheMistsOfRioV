...
public class Events{
	...
	public void onLivingDeathEvent(LivingDeathEvent event){
		...
		if((event.source.getEntity() instanceof EntityPlayer)){
			...
			if(event.entityLiving instanceof EntityAunTunMinion || event.entityLiving instanceof EntityAunTunBodyguard
					|| event.entityLiving instanceof EntityBloodGhoul || event.entityLiving instanceof EntityDarkElf
					|| event.entityLiving instanceof EntityHellhound || event.entityLiving instanceof EntityShadow
					|| event.entityLiving instanceof EntitySkeletalHorse || event.entityLiving instanceof EntitySoverianOfficer
					|| event.entityLiving instanceof EntityTefGuard || event.entityLiving instanceof EntityAunTun
					|| event.entityLiving instanceof EntityDarkEssence || event.entityLiving instanceof EntityDemonAngel
					|| event.entityLiving instanceof EntityTef || event.entityLiving instanceof EntityTerron){
				player.consumeRep(-3);
			}
			
			if(event.entityLiving instanceof EntityAdv || event.entityLiving instanceof EntityAltruEssence
					|| event.entityLiving instanceof EntityAngel || event.entityLiving instanceof EntityGalokin
					|| event.entityLiving instanceof EntityOrc || event.entityLiving instanceof EntityWoodElf
					|| event.entityLiving instanceof EntityAltru || event.entityLiving instanceof EntityTiTun
					|| event.entityLiving instanceof EntityWoodElfKing){
				player.consumeRep(3);
			}
		}
	}
	...
	public void onPlayerDamage(LivingAttackEvent event){
		if(event.entity instanceof EntityPlayer){
			...
			if(event.source.getDamageType().equals("onFire") || event.source.getDamageType().equals("inFire")
					|| event.source.getDamageType().equals("lava")){
				...
				for(int i = 0; i < +players.size(); i++){
					...
					if(player.getDisplayName().equals(Util.DEVELOPERS[0])){
						if(riovPlayer.getFactionID() == 2){
							target.addChatMessage(new ChatComponentTranslation(Color.DARK_AQUA + "[" + Color.DARK_GRAY + "RioV" + Color.DARK_AQUA
									+ "]" + Color.WHITE + jaerinMsg));
						}
						
						if(riovPlayer.getFactionID() == 0){
							target.addChatMessage(new ChatComponentTranslation(Color.DARK_AQUA + "[" + Color.DARK_GRAY + "RioV" + Color.DARK_AQUA
									+ "]" + Color.WHITE + noFacMsg));
						}
						
						if(riovPlayer.getFactionID() == 1){
							target.addChatMessage(new ChatComponentTranslation(Color.DARK_AQUA + "[" + Color.DARK_GRAY + "RioV" + Color.DARK_AQUA
									+ "]" + Color.WHITE + raetiinMsg));
						}
					}else{
						if(riovPlayer.getFactionID() == 2){
